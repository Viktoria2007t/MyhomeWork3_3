package com.example.myhomework3_4;
...
public class MainActivity extends AppCompatActivity implements MediaPlayer.OnPreparedListener, MediaPlayer.OnCompletionListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //присваеваем id кнопкам громче ,тихо
        Button btnVolumeUp = findViewById(R.id.btn_volume_up);
        Button btnVolumeDown = findViewById(R.id.btn_volume_down);
...
// Создаем экземпляр AudioManager
        AudioManager audioManager = (AudioManager) getSystemService(Context.AUDIO_SERVICE);

// Получаем максимальную громкость для потока звука медиа плеера
        int maxVolume = audioManager.getStreamMaxVolume(AudioManager.STREAM_MUSIC);

// Получаем текущую громкость для потока звука медиа плеера
        final int[][] currentVolume = {{audioManager.getStreamVolume(AudioManager.STREAM_MUSIC)}};
// Устанавливаем обработчик нажатия на кнопку громче
        btnVolumeUp.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                // Увеличиваем громкость на 1 шаг
                if (currentVolume[0][0] < maxVolume) {
                    currentVolume[0][0]++;
                    audioManager.setStreamVolume(AudioManager.STREAM_MUSIC, currentVolume[0][0], 0);
                }
            }
        });
        // Устанавливаем обработчик нажатия на кнопку тише
        btnVolumeDown.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                // Уменьшаем громкость на 1 шаг
